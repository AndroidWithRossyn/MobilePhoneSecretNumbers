...
import com.adsmodule.api.AdsModule.Retrofit.Interfaces.AppInterfaces;
import com.adsmodule.api.AdsModule.Retrofit.Utils.ConnectionDetector;
import com.adsmodule.api.AdsModule.Retrofit.Utils.Constants;
import com.adsmodule.api.AdsModule.Retrofit.Utils.Global;
import com.adsmodule.api.AdsModule.Retrofit.Utils.StringUtils;
...
*/
    ...
    public static void showNativeAd(Activity activity, String nativeAd, CardView adContainer, int isFullScreenAd, Drawable imageID) {
        cd = new ConnectionDetector(activity);
        Collections.shuffle(Constants.platformList);
        //show or hide ads according to value received
        if(!Constants.adsResponseModel.isShow_ads()) return;
        if (imageID != null) {
            adContainer.addView(Global.getDefaultImage(activity, imageID));
        }
        if (!isListNull(Constants.platformList)) {
            if (Constants.platformList.get(0).equals(Constants.ADX) && !isNull(Constants.adsResponseModel.getNative_ads().getAdx())) {
                if (cd.isConnectingToInternet() && Constants.adsResponseModel.isShow_ads()) {
                    try {
    ...
    private static void populateUnifiedNativeAdView(Activity activity, NativeAd unifiedNativeAd, NativeAdView unifiedNativeAdView, int flag) {
        ...
        if (unifiedNativeAd.getStarRating() == null) {
            ...
            ((RatingBar) unifiedNativeAdView.getStarRatingView()).setProgressBackgroundTintList(!isNull(Constants.adsResponseModel.getCommon_text_color()) ? ColorStateList.valueOf(Color.parseColor(Constants.adsResponseModel.getCommon_text_color())) : ColorStateList.valueOf(activity.getResources().getColor(R.color.black)));
        }
...
//        nativeAdSocialContext.setTextColor(Color.parseColor(Constants.adsResponseModel.getCommon_text_color()));
    ...
    public static void showAppOpenAds(String appopenAd, Activity activity, AppInterfaces.AppOpenADInterface appOpenADInterface) {
        ...
        if(Constants.platformList.isEmpty() && Constants.adsResponseModel.isShow_ads() ) {
        ...
        if (!isNull(appopenAd) && Constants.adsResponseModel != null && cd.isConnectingToInternet() && Constants.adsResponseModel.isShow_ads()) {
            if (StringUtils.CheckEqualIgnoreCase(Constants.adsResponseModel.getAds_open_type(), Constants.IS_APP_OPEN_ADS)) {
                Global.showAlertProgressDialog(activity);
                if (Constants.platformList.size() > 0 && Constants.platformList.get(0).equals(Constants.ADX) || Constants.platformList.get(0).equals(Constants.ADMOB)) {
                    if (precacheAppOpenAd != null) {
                        showAdIfAvailable(precacheAppOpenAd, activity, appOpenADInterface);
                    } else {
                        AdRequest adRequest = new AdRequest.Builder().build();
